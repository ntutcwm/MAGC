...
class SpacedSampler:
    ...
    def predict_noise(
        ...
        if uncond is None or cfg_scale == 1.:
            model_output = self.model.apply_model(x, t, cond, y_hat)
        else:
            # apply classifier-free guidance
            model_cond = self.model.apply_model(x, t, cond, y_hat)
            model_uncond = self.model.apply_model(x, t, uncond, y_hat)
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            ...
            # 这里
            e_t = self.predict_noise(
                x, t, cond, cfg_scale, uncond, y_hat
            )
    ...
    def sample_with_mixdiff(
        ...
        for i, step in enumerate(iterator):
            ...
            for hi, hi_end, wi, wi_end in tiles_iterator:
                ...
                tile_cond = {
                    "c_latent": [self.model.apply_condition_encoder(tile_cond_img)], # apply_condition_encoder的输入只能是ref_gt
    ...
    def sample(
        ...
        img_pixel = torch.clamp(img_pixel, 0, 1)
