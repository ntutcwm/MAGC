...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        super().__init__(conditioning_key=conditioning_key,parameterization=parameterization, *args, **kwargs)
    ...
    # def instantiate_hyper_stage(self):
    #     self.hyper = 
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x = super().get_input(batch, k) # img_gt
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach() # b 4 32 32
    ...
    def p_losses(self, x_start, cond, t, noise=None): # 训练时调用, cond['c_crossattn'], cond['c_ref']
        ...
        loss_simple = self.get_loss(model_output, target, mean=False).mean([1, 2, 3]) # loss是直接求mse，没有乘以系数
